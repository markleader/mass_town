...
def _run_with_thickness(tmp_path: Path, thickness: float):
    source = Path("examples/shell_modal_bdf_problem")
    base_dir = Path(
        tempfile.mkdtemp(
            prefix=f"mass_town_shell_modal_bdf_{str(thickness).replace('.', '_')}_"
        )
    )
    ...
    state.run_id = f"shell-modal-bdf-{str(thickness).replace('.', '-')}"
...
def test_shell_modal_bdf_example_reports_feasibility_and_frequency_trend(tmp_path: Path) -> None:
    ...
    assert thin_state.analysis_state.critical_frequency_hz is not None
    assert thick_state.analysis_state.critical_frequency_hz is not None
    assert (
        thick_state.analysis_state.critical_frequency_hz
        > thin_state.analysis_state.critical_frequency_hz
    )
    ...
    assert thin_summary["analysis_type"] == "modal"
    assert thick_summary["analysis_type"] == "modal"
    assert thin_summary["critical_natural_frequency_hz"] == pytest.approx(
        thin_state.analysis_state.critical_frequency_hz
    )
    assert thick_summary["critical_natural_frequency_hz"] == pytest.approx(
        thick_state.analysis_state.critical_frequency_hz
    )
    assert thin_summary["eigenvalue_constraints"]["minimum_natural_frequency_hz"]["passed"] is False
    assert thick_summary["eigenvalue_constraints"]["minimum_natural_frequency_hz"]["passed"] is True
